...
public class Coop_Activity extends AppCompatActivity {

    private Button buttonStart;
    private TextView textViewInstruction1, textViewTimer1, textViewInstruction2, textViewTimer2;
    final String INSTRUCTION_MESSAGE = "In this mode, you and a teammate need to click the buttons in ASCENDING order before the time runs out. The numbers could be on either side, so you need to work together. Caution: Both of you are only allowed 10 mistakes.";
    final int MAX_BUTTON_COUNT = 60;
    int numberToClick, failedAttempt, timeRemaining, showToastLost, showWinMessage, chosenSize;
    String player_One = "", teammate_Name = "";
    ...
    CountDownTimer countDownTimer = new CountDownTimer(121000, 1000) {
        ...
        public void onFinish() {
            ...
            if (showToastLost == 0) {
                lostSound.start();
                Toast.makeText(Coop_Activity.this, "Sorry, you run-out of time!", Toast.LENGTH_SHORT).show();
                showToastLost = 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_coop_);
    ...
    private void generateRandomNumbers() {
        ...
        backgroundSound = MediaPlayer.create(this, R.raw.coopbackgroundmusic);
    ...
    private void testIfCorrect(int x){
        if (buttonx[x].getText() == String.valueOf(numberToClick)) {
            ...
            if (failedAttempt >= 10) {
                if (showToastLost == 0) {
                    lostSound.start();
                    Toast.makeText(Coop_Activity.this, "FAILED! more than 10 failed attempts.", Toast.LENGTH_SHORT).show();
                    showToastLost = 1;
                }
                showHide();
            } else {
    ...
    private void acquireNameOfWinner() {
        ...
        builder.setTitle("First player's Name");
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                player_One = inputName.getText().toString();

                // input for second player's name
                AlertDialog.Builder builder2 = new AlertDialog.Builder(Coop_Activity.this);
                builder2.setCancelable(false);
                builder2.setTitle("Teammate's Name");
                // Set up the inputName
                final EditText inputName2 = new EditText(Coop_Activity.this);
                ...
                builder2.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        teammate_Name = inputName2.getText().toString();
                        ...
                        rootRef.child(keyScores).child("Name").setValue(player_One + " and " + teammate_Name);
                        rootRef.child(keyScores).child("Time").setValue(timeRemaining);
                        rootRef.child(keyScores).child("Date").setValue(DateToStr);
                        rootRef.child(keyScores).child("Mode").setValue("Coop");
                    }
                });
